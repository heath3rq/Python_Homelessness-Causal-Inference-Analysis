"""A module that merges data files."""
import pandas as pd
...
def check_dup_null(df: object) -> None:
    """Check if there is duplicates or null values and if data is consistent across years."""
    ...
    assert (
        df.groupby("CoC Number").count() == 15
    ).all().all() == True, (
        "Certain years are missing. Review yearly data for each CoC."  # 15 years
    )
...
print("Success: merged HIC and PIT data.")
...
def melt_df(dataframe: object, col_name: str) -> object:
    """Melt the dataframe to match the previous dataframe format."""
    df_melted = pd.melt(
...
check_dup_null(df_hic_pit_pov)
print("Success: merged poverty data with HIC and PIT.")
...
# Merge unemployment data with poverty & HIC & PIT
...
check_dup_null(df_hic_pit_pov_unemp)

print("Success: merged unemployment data with HIC, PIT, and poverty data.")


# Export file to csv
df_hic_pit_pov_unemp.to_csv("HIC_PIT_POV_UNEMP_2007_2021.csv")
print("Success: exported output file to csv.")
